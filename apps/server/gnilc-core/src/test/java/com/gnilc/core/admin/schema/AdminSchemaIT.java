package com.gnilc.core.admin.schema;
...
/** 重复执行管理员基线 SQL，验证身份和关系恢复不覆盖已有管理员数据或错误复活历史绑定。 */
@JdbcTest
@ActiveProfiles("test")
...
class AdminSchemaIT {
    ...
    /** 空库提供可用管理入口，重复部署不得恢复管理员主动解除的管理授权。 */
    @Test
    void freshInstallationGrantsManagementOnceWithoutRestoringRemovedAssignments() {
        ...
        jdbc.update("""
                ...
                JOIN sys_admin admin ON admin.user_id = binding.user_id
                SET binding.del = 1
    ...
    private java.util.List<String> defaultAdminRoles() {
        return jdbc.queryForList("""
                SELECT role.code FROM az_role role
                JOIN az_user_role binding ON binding.role_id = role.id AND binding.del = 0
                JOIN sys_admin admin ON admin.user_id = binding.user_id
                WHERE admin.username = 'admin' AND role.del = 0
                """, String.class);
    }
    ...
    @Test
    void adminSchemaDoesNotReactivateHistoricalBindingWhenActiveBindingExists() {
        runScript("sql/schema/02_admin.sql");
        Long userId = jdbc.queryForObject(
                "SELECT user_id FROM sys_admin WHERE username = 'admin'", Long.class);
        jdbc.update("""
                INSERT INTO az_user_role (
                    del, create_time, update_time, user_id, role_id
                )
                SELECT 1, UTC_TIMESTAMP(6), NULL, ?, id
                  FROM az_role
                 WHERE code = 'admin'
                """, userId);
        ...
        assertThat(defaultAdminRoleBindingCount(0)).isEqualTo(1);
        assertThat(defaultAdminRoleBindingCount(1)).isEqualTo(1);
    }
    ...
    private int defaultAdminRoleBindingCount() {
        return defaultAdminRoleBindingCount(0);
    }

    private int defaultAdminRoleBindingCount(int deleted) {
